...
%
%
%signal_dir='/Users/mk/Documents/MATLAB/training2017';
...
reffile = [signal_dir, filesep, 'REFERENCE.csv']; %original references
%reffile = [signal_dir, filesep, 'REFERENCE-v2.csv'];
    ...
    switch target{n}
        case 'N'
            ...
            elseif strcmp(this_answer,'A')
                AA(1,2) = AA(1,2)+1;
                N_FALSE_A = [N_FALSE_A, rec];
            elseif strcmp(this_answer,'O')
                AA(1,3) = AA(1,3)+1;
                N_FALSE_O = [N_FALSE_O, rec];
            elseif strcmp(this_answer,'~')
                AA(1,4) = AA(1,4)+1;
                N_FALSE_X = [N_FALSE_X, rec];
            end
        case 'A'
            if strcmp(this_answer,'N')
                AA(2,1) = AA(2,1)+1;
...
F1_phase_one=(F1n+F1a+F1o+F1p)/4;
F1=(F1n+F1a+F1o)/3;
